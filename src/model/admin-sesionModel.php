...
class SessionModel
{
    ...
    {
        // Primero verificamos que los parámetros no estén vacíos
        if (empty($id_sesion) || empty($token)) {
            error_log("Error: Parámetros vacíos en verificar_sesion_si_activa");
            return 0;
        }
        ...
        // Si la consulta falla, retornar 0
        if ($b_sesion === false) {
        ...
        if ($b_sesion->num_rows === 0) {
            return 0;
        }
        
        $datos_sesion = $b_sesion->fetch_object();
        
        // Verificar que el objeto se obtuvo correctamente
        if (!$datos_sesion) {
            return 0;
        }
        ...
        if ((password_verify($datos_sesion->token, $token)) && ($hora_actual <= $fecha_hora_fin)) {
            ...
            $update = $this->conexion->query("UPDATE sesiones SET fecha_hora_fin='$nueva_fecha_hora_fin' WHERE id=$id_sesion");
            
            // Verificar si el update fue exitoso
            if ($update === false) {
                error_log("Error al actualizar sesión: " . $this->conexion->error);
            }
